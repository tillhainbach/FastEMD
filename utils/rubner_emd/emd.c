...
#if MAX_SIG_SIZE > 1000
double* allocate_mem(double*** arr, int n, int m)
{
  *arr = (double**)malloc(n * sizeof(double*));
  double *arr_data = (double*)malloc( n * m * sizeof(double));
  for(int i=0; i<n; i++)
     (*arr)[i] = arr_data + i * m ;
  return arr_data; //free point
}

void deallocate_mem(double*** arr, double* arr_data){
    free(arr_data);
    free(*arr);
}
#endif
	  ...
	  flow_t *Flow, int *FlowSize)
{
  ...
  if (Flow != NULL)
    *FlowSize = static_cast<int>(FlowP - Flow);
...
static void newSol()
{
    ...
    double xMin;
    int steps;
    node2_t *Loop[2*MAX_SIG_SIZE1], *CurX, *LeaveX = nullptr;
...
static void russel(double *S, double *D)
{
  ...
  double deltaMin, oldVal, diff;
    ...
    double** Delta;
    double* Delta_data;
    Delta_data = allocate_mem(&Delta, MAX_SIG_SIZE1, MAX_SIG_SIZE1);
#else
  ...
  node1_t vHead, *CurV, *PrevV;
    node1_t *PrevUMinI = nullptr, *PrevVMinJ = nullptr, *Remember;
  ...
  do
    ...
    #if MAX_SIG_SIZE > 1000
    deallocate_mem(&Delta, Delta_data);
    #endif
}
